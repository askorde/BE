...
public:
    ...
    Node()
    {}
    Node(int a[3][3], int hv, int gv, Node *parent)
    {
    	cpy(c,a);
    	h=hv;g=gv;
    	p=parent;
...
void moves(int a[3][3], int goal[3][3], Graph g, int c, int id) //only h(n)
{
	if(!h(a,goal))
	{
		cout<<"\nReached Goal\n";
		...
		for(int j=0;j<3;j++)
		{
			if(a[i][j]==0)
			{
				if(i-1>=0){
					...
					hv=h(na1,goal);
					Node *n;
					n = new Node(na1,hv,sn->g+1, close.back());
					open.push_back(n);
				...
				if(i+1<3){
					...
					hv=h(na2,goal);
					Node *n2;
					n2 = new Node(na2,hv,sn->g+1, close.back());
					open.push_back(n2);
				...
				if(j-1>=0){
					...
					hv=h(na3,goal);
					Node *n3;
					n3 = new Node(na3,hv,sn->g+1, close.back());
					open.push_back(n3);
				...
				if(j+1<3){
					...
					hv=h(na4,goal);
					Node *n4;
					n4 = new Node(na4,hv,sn->g+1, close.back());
					open.push_back(n4);
